...
type Auth struct {
	...
	IsActive int `orm:"column(is_active);description(1:启用 0:停用)"`
	IsDelete int `orm:"column(is_delete);description(1:删除 0:未删除)"`
	Weight int `orm:"description(权重)"`
...
type Role struct {
	...
	IsActive int `orm:"column(is_active);description(启用状态)"`
	IsDelete int `orm:"column(is_delete);description(删除状态)"`
	CreateTime time.Time `orm:"type(datetime);auto_now;description(创建时间)"`
...
func init(){
	orm.RegisterModel(new(Auth),new(Role),new(User),new(CarBrand), new(Cars), new(CarsApply),new(MessageNotify))
}
